...
<script lang="ts">
  ...
  import { appwriteAccount } from "@c/appwrite/appwrite";

  import type { Models, RealtimeResponseEvent } from 'appwrite';


  export let _logs: Models.LogList | undefined;
  export let _sessions: Models.SessionList | undefined;

  let mounted = false;
  let settingLoad = false;
  let logs: any;
  let sessions: any;
  let skeleton: any;
  ...
  const userData = async () => {
    settingLoad = true;
	_logs = (await appwriteAccount.listLogs());
    logs = JSON.stringify(_logs);
	_sessions = (await appwriteAccount.listSessions());
